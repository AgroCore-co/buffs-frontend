...
export default function EquipeTab({ idPropriedade }: EquipeTabProps) {
  const t = useTranslations("Proprietario.detalhes.equipe");
  ...
  const renderCargoBadge = (cargo: Cargo) => {
    const badges: Record<string, { bg: string, text: string }> = {
      PROPRIETARIO: { bg: "bg-zinc-900", text: "text-white" },
      GERENTE: { bg: "bg-blue-100", text: "text-blue-700" },
      VETERINARIO: { bg: "bg-teal-100", text: "text-teal-700" },
      FUNCIONARIO: { bg: "bg-emerald-100", text: "text-emerald-700" },
    };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      >
        <TableHeader>
          <TableHead>{t("headers.member")}</TableHead>
          <TableHead>{t("headers.contact")}</TableHead>
          <TableHead>{t("headers.role")}</TableHead>
          <TableHead align="right">{t("headers.actions")}</TableHead>
        </TableHeader>
        <TableBody>
        ...
        <div className="fixed inset-0 z-[999] flex items-center justify-center bg-zinc-900/40 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden border border-zinc-200 animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between p-5 border-b border-zinc-100">
              <div>
                <h3 className="text-base font-semibold text-zinc-900">{t("editRole.title")}</h3>
                <p className="text-xs text-zinc-500 mt-0.5">{t("editRole.subtitle", { name: selectedUser.nome })}</p>
              </div>
            ...
            <form onSubmit={handleUpdateCargo} className="p-5 space-y-4">
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-zinc-700">{t("editRole.selectRole")}</label>
                <select
                  required
                  value={selectedCargo}
                ...
                >
                  <option value="" disabled>{t("editRole.selectPlaceholder")}</option>
                  <option value="GERENTE">{t("editRole.options.GERENTE")}</option>
                  <option value="VETERINARIO">{t("editRole.options.VETERINARIO")}</option>
                  <option value="FUNCIONARIO">{t("editRole.options.FUNCIONARIO")}</option>
                </select>
      ...
      {isUnlinkModalOpen && selectedUser && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center bg-zinc-900/40 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden border border-zinc-200 animate-in zoom-in-95 duration-200 p-6 text-center">
            ...
            <h3 className="text-lg font-semibold text-zinc-900 mb-1">{t("unlink.title")}</h3>
            <p className="text-sm text-zinc-500 mb-6">
              {t("unlink.description", { name: selectedUser.nome })}
            </p>
