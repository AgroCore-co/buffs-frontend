...
import { FiEdit2, FiTrash2, FiMapPin, FiUser, FiCalendar } from "react-icons/fi";
...
export default function PropriedadeCard({ propriedade, onEditar, onDeletar }) {
  ...
  return (
    <div 
      className="group relative bg-white border border-gray-200 rounded-xl hover:border-[#ffcf78] hover:shadow-lg transition-all duration-300 flex flex-col justify-between overflow-hidden"
    >
      ...
      >
        <div className="flex justify-between items-start mb-4">
          <div className="flex gap-2 flex-wrap">
            ...
            {propriedade.p_abcb && (
              <Badge type="info">ABCB</Badge>
            )}
          </div>
        </div>
        <div className="mb-5">
          <h3 className="text-lg font-bold text-[#404040] mb-1 group-hover:text-[#ce7d0a] transition-colors line-clamp-1" title={propriedade.nome}>
            {propriedade.nome}
          </h3>
        ...
        <div className="grid grid-cols-2 gap-3 mb-5 mt-auto">
          <div className="bg-[#f8fcfa] p-2.5 rounded-lg border border-gray-100 group-hover:border-[#ffcf78]/30 transition-colors">
            <span className="text-[10px] text-gray-400 uppercase font-bold block mb-1">Manejo</span>
            <span className="text-xs font-semibold text-[#404040] block truncate">
              {getManejoLabel(propriedade.tipo_manejo)}
            </span>
          </div>
          <div className="bg-[#f8fcfa] p-2.5 rounded-lg border border-gray-100 group-hover:border-[#ffcf78]/30 transition-colors">
            <span className="text-[10px] text-gray-400 uppercase font-bold block mb-1">Atualização</span>
        ...
        <div className="pt-4 border-t border-gray-100 flex flex-col gap-2.5">
          <div className="flex items-start gap-2.5">
            ...
            <span className="text-xs text-gray-600 leading-snug line-clamp-2">
              {propriedade.endereco ? `${propriedade.endereco.bairro}, ${propriedade.endereco.cidade} - ${propriedade.endereco.estado}` : "Endereço não cadastrado"}
            </span>
          </div>
