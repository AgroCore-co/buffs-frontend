...
function CicloOrdenhas({ idCiclo }: { idCiclo: string }) {
  ...
  if (ordenhas.length === 0) {
    return <p className="text-center text-xs text-zinc-400 py-6">Nenhuma ordenha neste ciclo.</p>;
  }

  return (
    <div>
      <div className="bg-zinc-50/60 px-4 py-2 flex justify-between items-center border-b border-zinc-100">
        <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-400">Registros do Ciclo</span>
      ...
      >
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${ciclo.atual ? "bg-emerald-100 text-emerald-700" : "bg-zinc-100 text-zinc-500"}`}>
                {ciclo.atual ? "Em lactação" : "Encerrado"}
              </span>
            </div>
        ...
        <div className="flex items-center gap-4">
          <div className="text-right hidden sm:block">
            <span className="text-[10px] text-zinc-400 block uppercase tracking-wide">Total produzido</span>
  ...
  const ciclos = useMemo<CicloItem[]>(() => {
    ...
    if (cicloAtual) {
      lista.push({
        ...
        periodo: `${year(cicloAtual.dtParto)} – em curso`,
    ...
    <div className="bg-white border border-zinc-200 rounded-2xl p-5">
      ...
      {ciclos.length === 0 ? (
        <div className="flex flex-col items-center justify-center gap-2 py-10 text-zinc-300">
          ...
          <span className="text-sm">Nenhum ciclo de lactação registrado</span>
        </div>
      ) : (
