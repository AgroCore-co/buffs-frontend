...
export function DeletedVacinacaoModal({ isOpen, onClose, idBufalo, onMutated }: Props) {
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title="Vacinações Removidas"
      description="Registros de vacinação removidos deste búfalo. Restaure qualquer um para reativá-lo."
      size="lg"
    >
      {isLoading && (
      ...
      {!isLoading && deletados.length === 0 && (
        <div className="flex flex-col items-center justify-center h-40 gap-3 text-center">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-400 mt-0.5">Este búfalo não possui vacinações removidas.</p>
          </div>
        </div>
      )}
