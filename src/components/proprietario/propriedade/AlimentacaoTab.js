...
export default function AlimentacaoTab({
  ...
  const definicoes = [
    {
      id_aliment_def: 1,
      tipo_alimentacao: 'Ração de Crescimento',
      descricao: 'Ração balanceada para fase inicial',
    },
    {
      id_aliment_def: 2,
      tipo_alimentacao: 'Suplemento Mineral',
      descricao: 'Sal mineral com fósforo',
    },
  ];
      ...
      <DashboardContainer>
        ...
        >
          ...
          <MetricCard
            ...
            value={`${consumoDiarioTotal.toFixed(1)} kg`}
            subtitle="estimado"
          />
          <MetricCard
            title="Estoque Crítico"
            value={estoqueCritico}
            subtitle="abaixo de 100 kg"
          />
          <MetricCard
            title="Total de Registros"
            value={totalRegistros}
            subtitle="de alimentação"
          />
        </div>
      </DashboardContainer>
      ...
      <DashboardContainer>
        <Table
          ...
          renderCell={(def, key) => {
            ...
            if (key === 'descricao')
              return (
                <span title={def.descricao} className="max-w-md truncate block">
                  {def.descricao}
                </span>
              );
            return def[key];
