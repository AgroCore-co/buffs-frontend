...
export default function MapaDesenhoLeaflet({
    ...
    useEffect(() => {
        ...
        if (initialPolygon?.coordinates?.[0]?.length > 0) {
            ...
            const bounds = L.latLngBounds(allLatLngs.length > 0 ? allLatLngs : latLngs);
        ...
        existingLotes
            ...
            .forEach((lote) => {
                ...
                polygon.bindTooltip(lote.nome_lote, {
                    permanent: false,
                    direction: 'center',
                    className: 'bg-white/90 backdrop-blur px-2 py-1 rounded text-xs font-medium',
                });
            });
            ...
            {!isLocked && (
                <div className="absolute bottom-3 left-3 right-3 z-[1000]">
                    <div className="bg-white/95 backdrop-blur-sm rounded-lg px-4 py-2 shadow-lg text-sm text-slate-600">
                        ...
                        {points.length === 1 && (
                            <span>👆 Continue clicando para desenhar - {points.length} ponto</span>
                        )}
                        {points.length === 2 && (
                        ...
                        {points.length >= 3 && (
                            <span>
                                ✅ {points.length} pontos - Clique em &quot;Finalizar Desenho&quot; para confirmar
                            </span>
                        )}
                    </div>
                </div>
