...
const getStatusColor = (status) => status ? 'bg-green-100 text-green-700 border-green-200' : 'bg-red-100 text-red-700 border-red-200';
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          <div className="flex items-center gap-5">
            <div className="w-16 h-16 rounded-full bg-amber-100 flex items-center justify-center border-4 border-white shadow-sm shrink-0">
               <span className="text-2xl font-bold text-amber-700">{bufalo.nome.substring(0,2).toUpperCase()}</span>
            </div>
            <div>
              <div className="flex items-center flex-wrap gap-3">
                <h1 className="text-2xl font-bold text-slate-900">{bufalo.nome}</h1>
                <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold border ${getStatusColor(bufalo.status)}`}>
                ...
                {bufalo.categoria && (
                   <div className="flex items-center gap-1 bg-slate-100 px-2 py-0.5 rounded-full border border-slate-200">
                      <span className="text-xs font-bold text-slate-600">{bufalo.categoria}</span>
                   </div>
                )}
              </div>
              <div className="flex items-center gap-4 mt-1 text-sm text-slate-500">
                <span className="flex items-center gap-1"><Tag className="w-3 h-3" /> Brinco: <strong className="text-slate-700">{bufalo.brinco}</strong></span>
                <span className="hidden md:inline text-slate-300">|</span>
                <span className="flex items-center gap-1"><Dna className="w-3 h-3" /> Raça: {bufalo.raca?.nome}</span>
                <span className="hidden md:inline text-slate-300">|</span>
                <span className="flex items-center gap-1"><Calendar className="w-3 h-3" /> {calculateAge(bufalo.dt_nascimento)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            <Card>
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="text-xs text-slate-400">ID: {bufalo.id_bufalo.split('-')[0]}...</div>
              </div>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <InfoItem icon={Tag} label="Brinco Visual" value={bufalo.brinco} />
                <InfoItem icon={QrCode} label="Microchip / Eletrônico" value={bufalo.microchip || "Não implantado"} />
                <InfoItem icon={Baby} label="Data Nascimento" value={formatDate(bufalo.dt_nascimento)} subValue={`${calculateAge(bufalo.dt_nascimento)} de idade`} />
                <InfoItem icon={Dna} label="Sexo" value={getSexLabel(bufalo.sexo)} />
                <InfoItem icon={Tag} label="Registro Provisório" value={bufalo.registro_prov} />
                <InfoItem icon={Tag} label="Registro Definitivo" value={bufalo.registro_def} />
                <InfoItem icon={MapPin} label="Origem" value={bufalo.origem} />
                <InfoItem icon={Tag} label="Brinco Original" value={bufalo.brinco_original} />
              </div>
            </Card>
            ...
            <Card>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="p-4 bg-slate-50 rounded-lg border border-slate-100">
                  <p className="text-xs text-slate-500 uppercase font-semibold mb-1">Propriedade</p>
                  <p className="text-lg font-medium text-slate-800">{bufalo.propriedade?.nome || "Sem propriedade"}</p>
                </div>
                <div className="p-4 bg-slate-50 rounded-lg border border-slate-100">
                  <p className="text-xs text-slate-500 uppercase font-semibold mb-1">Grupo / Lote</p>
                  <p className="text-lg font-medium text-slate-800">{bufalo.grupo?.nome_grupo || "Sem grupo definido"}</p>
                </div>
                ...
                <div className="p-4 bg-slate-50 rounded-lg border border-slate-100">
                   ...
                   <p className="text-lg font-medium text-slate-800">{bufalo.categoria || 'SRD'}</p>
                </div>

                <div className="p-4 bg-slate-50 rounded-lg border border-slate-100">
                  <p className="text-xs text-slate-500 uppercase font-semibold mb-1">Nível Maturidade</p>
                  <p className="text-lg font-medium text-slate-800">
                    {bufalo.nivel_maturidade === 'V' ? 'Vaca / Adulto' : bufalo.nivel_maturidade}
                  </p>
                </div>
              </div>
            </Card>
          ...
          <div className="space-y-6">
            ...
            <Card className="flex flex-col items-center justify-center text-center py-10 relative overflow-hidden border-amber-200">
               ...
               <div className="relative z-10 w-full flex flex-col items-center">
                  <h4 className="text-xs font-bold text-amber-600 uppercase tracking-widest mb-6">Classificação Racial</h4>
                  ...
                  <h3 className="text-3xl font-extrabold text-slate-800 mb-2">{bufalo.categoria || 'SRD'}</h3>
                  <div className="px-4 py-1.5 rounded-full bg-slate-100 border border-slate-200">
                     <p className="text-sm font-medium text-slate-600">{getCategoriaLabel(bufalo.categoria)}</p>
                  </div>
               </div>
            </Card>
            ...
            <Card>
              ...
              <div className="relative border-l-2 border-slate-200 ml-4 pl-8 space-y-6 py-2">
                ...
                <div className="relative">
                  ...
                  <div>
                    <p className="text-xs text-slate-500 uppercase leading-none mb-1">Pai</p>
                    <p className="font-medium text-slate-800">
                      {bufalo.id_pai_semen ? 'Inseminação' : (bufalo.id_pai ? 'Touro Local' : 'Não informado')}
                    </p>
                  </div>
                </div>
                ...
                <div className="relative">
                  ...
                  <div>
                    <p className="text-xs text-slate-500 uppercase leading-none mb-1">Mãe</p>
                    <p className="font-medium text-slate-800">
             ...
             <div className="text-xs text-slate-400 space-y-1 px-2">
                <p className="flex items-center gap-1"><Clock className="w-3 h-3" /> Criado em: {bufalo.created_at}</p>
                <p className="flex items-center gap-1"><Edit className="w-3 h-3" /> Atualizado: {bufalo.updated_at}</p>
             </div>
          </div>
        </div>
      )}
